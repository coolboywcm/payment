  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<table>
				<tr>
					...
					<%-- <td style="vertical-align:top;"><a class="btn btn-small btn-light" onclick="window.location.href='<%=basePath%>/user/listtabUsers.do';" title="切换模式"><i id="nav-search-icon" class="icon-exchange"></i></a></td>
					...
					<c:when test="${not empty userList}">
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<td>${user.name }</td>
								<td>${user.email }&nbsp;<i class="icon-envelope"></i></a></td>
								<td>${user.lastLogin}</td>
								<td>${user.ip}</td>
								<c:if test="${QX.edit == 1 ||  QX.del == 1}">
									<td style="width: 60px;">
										<div class='hidden-phone visible-desktop btn-group'>
											<c:if test="${QX.edit == 1 && user.username != 'admin'}">
												<a class='btn btn-mini btn-info' title="编辑" onclick="editUser('${user.userId }');"><i class='icon-edit'></i></a>
											</c:if>
											<c:if test="${QX.del == 1 && user.username != 'admin' }">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>user/goEditU?userId='+user_id;
			 diag.Width = 355;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
