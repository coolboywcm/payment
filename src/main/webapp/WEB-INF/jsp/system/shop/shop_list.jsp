...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="shop/listShops" method="post" name="qryForm" id="qryForm">
			<table>
				<tr>
					<td>
					...
					<%-- <td style="vertical-align:top;"><a class="btn btn-small btn-light" onclick="window.location.href='<%=basePath%>/shop/listtabUsers.do';" title="切换模式"><i id="nav-search-icon" class="icon-exchange"></i></a></td>
					<td style="vertical-align:top;"><a class="btn btn-small btn-light" onclick="toExcel();" title="导出到EXCEL"><i id="nav-search-icon" class="icon-download-alt"></i></a></td>
						...
						<c:forEach items="${shopList}" var="shop" varStatus="vs">
							...
							<tr>
								<td class='center' style="width: 30px;">
									<c:if test="${shop.name != 'admin'}"><label><input type='checkbox' name='ids' value="${shop.id }" id="${shop.id }" alt="${shop.name }"/><span class="lbl"></span></label></c:if>
									<c:if test="${shop.name == 'admin'}"><label><input type='checkbox' disabled="disabled" /><span class="lbl"></span></label></c:if>
								</td>
								<td class='center' style="width: 30px;">${vs.index+1}</td>
								<td>${shop.orgName }</td>
								<td>${shop.code }</td>
								<td>${shop.terminalCount }</td>
								<td>${shop.name }</td>
								...
								<c:if test="${QX.edit == 1 ||  QX.del == 1}">
									<td style="width: 60px;">
										<div class='hidden-phone visible-desktop btn-group'>
											<c:if test="${QX.edit == 1 }">
												<a class='btn btn-mini btn-info' title="编辑" onclick="editShop('${shop.id }');"><i class='icon-edit'></i></a>
											</c:if>
											<c:if test="${QX.del == 1 }">
												 <a class='btn btn-mini btn-danger' title="删除" onclick="delshop('${shop.id }','${shop.name }');"><i class='icon-trash'></i></a>
											</c:if>
										</div>
									</td>
								</c:if>	
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function sendSms(phone){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送短信";
			 diag.URL = '<%=basePath%>head/goSendSms.do?lastLogin='+phone+'&msg=appuser';
			 diag.Width = 600;
			 diag.Height = 265;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function editShop(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>shop/goEdit?id='+id;
			 diag.Width = 325;
			 diag.Height = 460;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delshop(id,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>shop/del?id="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>shop/deleteAll?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>shop/excel.do?USERNAME='+USERNAME+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID;
		}
		...
		function fromExcel(){
			 ...
			 diag.URL = '<%=basePath%>shop/goUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
