...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="adimg/listAdimg" method="post" name="qryForm" id="qryForm">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${list}" var="bean" varStatus="vs">
							...
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${bean.id }" id="${bean.id }" alt="${bean.name }"/><span class="lbl"></span></label>
								</td>
								...
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										<c:if test="${QX.edit == 1 }">
										    <a class='btn btn-mini btn-info' title="编辑" onclick="editBonus('${bean.id }');"><i class='icon-edit'></i></a>
										</c:if>
										<c:if test="${QX.del == 1 }">
										 	<a class='btn btn-mini btn-danger' title="删除" onclick="delbonus('${bean.id }','${bean.name }');"><i class='icon-trash'></i></a>
										</c:if>
									</div>
								</td>
								</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = "<%=basePath%>adimg/edit?type=add";
			 diag.Width = 600;
			 diag.Height = 600;
		...
		function editBonus(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>adimg/edit?type=edit&id='+id;
			 diag.Width = 600;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delbonus(id,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>adimg/del?id="+id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>adimg/deleteAll?tm='+new Date().getTime(),
						    	data: {ids:str},
								dataType:'json',
