...
public class SqlHelper extends DynamicSql<SqlHelper> {
		...
		if (cond.conds != null) {
			for (Object[] op : cond.conds) {
				switch ((SqlCondition.Op) op[0]) {
					...
					case condition: condition((String)op[1]); break;
	...
	private final ConcurrentHashMap<Class<?>, Map<String, Field>> classFields = new ConcurrentHashMap<Class<?>, Map<String, Field>>();
	...
	public <M> List<M> fetch(Class<M> type) throws Exception {
		...
		List<M> rs = new ArrayList<M>(results.size());
	...
	private void assembleResult(Map<String, Object> result, Object model,
		...
		for (Map.Entry<String, Field> fe : modelFields.entrySet()) {
			...
			if (isBasicType(fieldType)) {
				...
				if(result.keySet().contains(field.getName())){
	...
	private static final Map<CacheLevel, Cache<CacheKey, List<Map<String, Object>>>> caches = 
			new HashMap<CacheLevel, Cache<CacheKey, List<Map<String, Object>>>>();
	...
	public List<Map<String, Object>> fetch(final EnumSet<KeyStrategy> ks) throws Exception {
		...
		if (this.cacheLevel != null) {
			...
			result = caches.get(this.cacheLevel)
							.get(new CacheKey(sql, param, ks), new Callable<List<Map<String, Object>>>() {
								@Override
	...
	public SqlHelper onResult(ResultHandler rh) {
		if (this.resultHandlers == null)
			this.resultHandlers = new ArrayList<ResultHandler>();
	...
	private List<Map<String, Object>> handleResult(List<Map<String, Object>> result, EnumSet<KeyStrategy> ks) {
		if (result != null) {
			if (ks != null && (ks.contains(KeyStrategy.UPPER_CASE) || ks.contains(KeyStrategy.CAMEL))) {
				List<Map<String, Object>> newR = new ArrayList<Map<String, Object>>();
	...
	public static SqlHelper create(NamedParamQuery jdbc, String querySql) {
		if(querySql == null){
			throw new RuntimeException("query sql can't be null");
		}
		return new SqlHelper(jdbc, new StringBuilder( querySql ), null);
	}

	public SqlHelper(NamedParamQuery jdbc, StringBuilder cond, Map<String, Object> paramMap) {
		super(cond, paramMap);
		if(jdbc == null){
			throw new RuntimeException("jdbc template can't be null");
		}
		this.jdbc = jdbc;
	}
